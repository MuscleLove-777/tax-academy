...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('tax-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('tax-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('tax-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('tax-progress', JSON.stringify(this.progress));
        localStorage.setItem('tax-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>節税 Academy へようこそ</h2>' +
            '<p>個人・法人の合法的な節税スキームを体系的に学べるプラットフォームです。</p>' +
            '</div>' +
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('tax-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('tax-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('tax-progress');
            localStorage.removeItem('tax-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
